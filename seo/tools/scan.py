...
import csv,json,re,hashlib,collections,datetime,urllib.parse as U,xml.etree.ElementTree as ET
from bs4 import BeautifulSoup
ROOT=Path(__file__).resolve().parents[2]
OUT=ROOT/'seo'
BASE='https://www.powertech-performance.com'
implementation=json.loads((OUT/'implementation.json').read_text('utf8')) if (OUT/'implementation.json').exists() else {}
...
evidence=dict(scanned_at=datetime.datetime.now(datetime.timezone.utc).isoformat(),scope='lokaler Projektstand; keine Aussage über Veröffentlichung',sources=sources,pages=pages,sitemap=sitemap,robots=(ROOT/'robots.txt').read_text(),config=cfg,performance=matches,assets=assets)
